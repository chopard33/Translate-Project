...
namespace KoreanAIO.Model
{
    ...
    public class SpellBase
    {
        ...
        {
            if (!MenuManager.SubMenus.ContainsKey("预判"))
            {
                MenuManager.AddSubMenu("预判");
            }
            Slider = MenuManager.GetSubMenu("预判").Add(SlotName, new Slider(SlotName + " 命中率百分比", defaultValue > 0 ? defaultValue : (int)MinHitChancePercent));
            return this;
        }

        public SpellBase AddDrawings(bool defaultValue = true, ColorBGRA? color = null)
        {
            var checkBox = MenuManager.GetSubMenu("线圈").Add(SlotName, new CheckBox("显示 " + SlotName + " 范围", defaultValue));
