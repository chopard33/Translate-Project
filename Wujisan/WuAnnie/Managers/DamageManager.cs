...
namespace WuAIO.Managers
{
    class DamageManager
    {
        ...
        private bool _enabled { get { return MenuManager.Menus[HeroBase.HERO_MENU]["线圈"].Keys.Last().IsActive("damageindicator"); } }
        ...
        private void Drawing_OnEndScene(System.EventArgs args)
        {
            ...
            foreach (var enemy in EntityManager.Heroes.Enemies.Where(it => it.IsValidTarget() && !it.IsDead && it.IsHPBarRendered))
            {
                ...
                if (FutureDamage == -1)
                {
                    Drawing.DrawText(enemy.Position.WorldToScreen().X - 30, enemy.Position.WorldToScreen().Y - 150, Color.Yellow, "可击杀");
                    continue;
                }
